...
const vitalidade = {
  ...
  statusAluno: [
    ...
    {
      label: "Em riscos (Faltas)",
      valor: 12,
      estilo: "text-red-500 bg-red-50",
      destaque: true,
    },
    {
...
function HomeDiretor() {
  return (
    <div className="flex h-screen bg-[#F8FAFC] overflow-hidden font-sans">
      <Sidebar tipoUsuario="Administrador" />
            ...
            <div className="flex flex-col xl:flex-row gap-8 items-start">
              ...
              <div className="flex-1 flex flex-col gap-8 min-w-0 w-full">
                ...
                <Acoes
                  acoes={[
                    {
                      id: "dashboard-completa",
                      label: "Dashboard Completa",
                      subLabel: "Ver relatório do sistema",
                      icon: LayoutGrid,
                      isPrimary: true,
                      onClick: () =>
                        console.log("Abrindo Dashboard Completa..."),
                    },
                    {
                      ...
                      icon: Users,
                      onClick: () =>
                        console.log("Abrindo Gerenciar Usuários..."),
                    },
                    {
                      id: "consultar-alunos",
                      label: "Consultar Alunos",
                      icon: UserSearch,
                      onClick: () => console.log("Abrindo Consultar Alunos..."),
                    },
                  ]}
                />
                  ...
                  <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                    ...
                    <div className="flex divide-x divide-gray-100 border-b border-gray-100">
                      {vitalidade.stats.map((stat, i) => (
                        <div
                          key={i}
                          className="flex-1 p-5 flex flex-col items-center gap-1"
                        >
                          <span className={`text-2xl font-black ${stat.cor}`}>
                            {stat.valor}
                          </span>
                          <span className="text-[10px] text-gray-400 font-semibold uppercase tracking-wider text-center">
                            {stat.label}
                          </span>
                        </div>
                      ))}
                        ...
                        <table className="w-full">
                          <tbody>
                            {vitalidade.statusAluno.map((item, i) => (
                              ...
                              >
                                <td className="py-2 text-[12px] text-gray-600">
                                  {item.label}
                                </td>
                                <td className="py-2 text-right">
                                  {item.destaque ? (
                                    <span
                                      className={`text-[12px] font-bold px-2 py-0.5 rounded ${item.estilo}`}
                                    >
                                      {item.valor}
                                    </span>
                                  ) : (
